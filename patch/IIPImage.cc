...
void IIPImage::testImageType()
{
  ...
  if( (stat(pstr,&sb)==0) && S_ISREG(sb.st_mode) ){
...
#ifdef HAVE_OPENSLIDE
    if( openslide_detect_vendor(pstr) != NULL ){
      format = ImageEncoding::OPENSLIDE;
      isFile = true;
    ...
    // Check for sequence
    glob_t gdat;
    string filename = path + fileNamePattern + "001_090.*";
...
void IIPImage::testImageType() {
  ...
  // If it's an SVS, we are NOT a sequence. Period.
  if (path.find(".svs") != string::npos) {
    this->isFile = true; 
    this->format = ImageEncoding::OPENSLIDE;
    if (stat(pstr, &sb) == 0) this->timestamp = sb.st_mtime;
    else this->timestamp = time(NULL);
    return; 
  }

  // ... rest of your existing stat and magic byte logic ...
}
...
void IIPImage::measureVerticalAngles()
{
  ...
  string filename = fileSystemPrefix + imagePath + fileNamePattern + "001_*." + suffix;
...
void IIPImage::measureHorizontalAngles()
{
  ...
  string filename = fileSystemPrefix + imagePath + fileNamePattern + "*_091." + suffix;
