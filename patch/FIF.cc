...
/*****************************************************
      Test for supported image formats: TIFF, JPEG or OpenSlide
    ******************************************************/

    ImageEncoding format = test.getImageFormat();
#if defined(HAVE_OPENSLIDE)
    if( format == ImageEncoding::OPENSLIDE ){
        if( session->loglevel >= 2 ) 
            *(session->logfile) << "FIF :: OpenSlide-compatible image detected" << endl;
    ...
    if ( format == ImageEncoding::OPENSLIDE ) {
         (*session->image)->loadImageInfo( 0, 0 );
    }
