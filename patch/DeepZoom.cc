...
void DeepZoom::run( Session* session, const std::string& argument ){
  ...
  string prefix, suffix, tile_ext;
  suffix = argument.substr( argument.find_last_of( "." )+1, argument.length() );

  // Save the tile format extension before suffix gets overwritten during coordinate parsing
  tile_ext = suffix;
  ...
  // Set output format based on the tile file extension in the request URL
#ifdef HAVE_PNG
  if( tile_ext == "png" ) session->view->output_format = ImageEncoding::PNG;
#endif
